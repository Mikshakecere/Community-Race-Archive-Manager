...
class SheetsManager:
    ...
    def lb_to_t3(self) -> None:
        ...
        processes = []
        for i in range(3):
            process = mp.Process(target=self.top_3_updater,args=(places[f"placed{i}"],66+(i*3)))
            process.start()
    ...
    def auto_format(self) -> str:
        row = 2*(self.grab_latest()-1)+8
        print(row)
        raceTitle = self.sheets.values().get(spreadsheetId=self.ssid,
                                            range=f"Leaderboard!C{row}").execute().get("values")[0][0]
        raceHost = self.sheets.values().get(spreadsheetId=self.ssid,
                                            range=f"Leaderboard!E{row}").execute().get("values")[0][0]
        return f"Race #{self.grab_latest()}: \"{raceTitle}\" by {raceHost}"